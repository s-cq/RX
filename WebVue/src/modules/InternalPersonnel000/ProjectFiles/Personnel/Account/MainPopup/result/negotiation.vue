<template>
    <div class="layerRtb layerRtb-threecolumn">
        <three-title :title="{name:'洽商'}"></three-title>
        <div class="layerRtb-scroll thinScroll" v-scrollHeight="137">
            <div class="analyItem" v-for="(item,index) in completionList" :key="index">
                ...
                <div class="analyItemCon">
                    ...
                    <p class="col-md-3">
                        <span class="cLightGray pr8">洽商利润</span>
                        <span>{{item.negotiate_profit.toFixed(2)}}</span>
                    </p>
                    <p class="col-md-4">
                    ...
                    <p class="col-md-3">
                        ...
                        <span>{{item.negotiate_formula | toSplict}}</span>
                    </p>
                    <p class="col-md-3">
                        <span class="cLightGray pr8">实际提成</span>
                        <span title="">{{item.comprehensive_negotiate.toFixed(2)}}</span>
                    </p>
    ...
    methods: {
        load () {
            for (var i = 0; i < this.completionList.length; i++) {
                this.countSum++
                this.completion_money += this.completionList[i].negotiate_profit
                this.comprehensive_completion += this.completionList[i].comprehensive_negotiate
            }
        },
