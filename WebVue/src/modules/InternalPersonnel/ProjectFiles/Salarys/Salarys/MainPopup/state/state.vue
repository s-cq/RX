<template>
    <div class="layerRtb layerRtb-threecolumn">
        ...
        <div class="layerRtb-scroll thinScroll" v-scrollHeight="270">
            ...
            <div class="analyItem">
                ...
                <div class="analyItemCon">
                    ...
                    <p class="col-md-4">
                        ...
                        <span>{{(Number(salaryModel.Salary)+Number(salaryModel.AdjustMoney)).toFixed(2)}}</span>
                    </p>
                    <p class="col-md-4">
                    ...
                    <p class="col-md-4">
                        ...
                        <span>{{salaryModel.Commission == null ? 0 : salaryModel.Commission.toFixed(2)}}</span>
                    </p>
                    <div class="fl col-md-3">
            ...
            <div class="analyItem" v-for="(item, index)  in mondataList" :key="index">
...
import { getInternalSalaryByUserCard, updateProvideSalary, pushCheckedSalaryToTouziCaiwu, getMonthReviewRecording } from '../../Resources/Api'
// 手动平衡操作
var gc = true
// 手动通过操作
var tg = true
        ...
        getMonthReviewRecording () {
            ...
            getMonthReviewRecording(parms).then((results) => {
                if (results.data.StatusCode === 0) {
                    ...
                    // 结果
                    const resultData = data.filter((item) => { return Number(item.evaluate_type) === 5 })
                    if (resultData.length) {
                        const currentData = resultData.sort(function (a, b) {
                            return a.create_date < b.create_date ? 1 : -1
                        })
                        this.mondataList.push(currentData[0])
                    }
                    // 过程
                    const processData = data.filter((item) => { return Number(item.evaluate_type) === 22 })
                    if (processData.length) {
                        const currentData = processData.sort(function (a, b) {
                            return a.create_date < b.create_date ? 1 : -1
                        })
                        this.mondataList.push(currentData[0])
                    }
                    // 管理
                    const manData = data.filter((item) => { return Number(item.evaluate_type) === 20 })
                    if (manData.length) {
                        const currentData = manData.sort(function (a, b) {
                            return a.create_date < b.create_date ? 1 : -1
                        })
                        this.mondataList.push(currentData[0])
                    }
