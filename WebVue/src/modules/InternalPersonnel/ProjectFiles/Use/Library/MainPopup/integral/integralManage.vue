...
export default {
    ...
    methods: {
        load () {
            // 增项
            getWarehouseResultDetail({ abilityLevel: this.leftInfo.grade, abilityType: 96 }).then((result) => {
        ...
        UdateStayLevelYJ () {
            ...
            this.$set(this.userLevelStandardDetail, 'abilityType', 96)
        ...
        UdateStayLevel () {
            getWarehouseResultDetail({ abilityLevel: this.leftInfo.grade, abilityType: 96, greatMore: this.greatMore }).then((result) => {
                if (result.data.StatusCode === 0) {
                    this.$message({
                        type: 'info',
                        message: '修改成功'
