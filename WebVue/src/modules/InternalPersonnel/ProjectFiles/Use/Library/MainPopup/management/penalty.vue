...
export default {
    ...
    methods: {
        ...
        UdateStayLevelYJ () {
            ...
            this.$set(this.userLevelStandardDetail, 'abilityType', 9)
        ...
        UdateStayLevel () {
            getWarehouseResultDetail({ abilityLevel: this.leftInfo.grade, abilityType: 9, greatMore: this.greatMore }).then((result) => {
                if (result.data.StatusCode === 0) {
                    this.$message({
                        type: 'info',
                        message: '修改成功'
