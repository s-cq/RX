            ...
            if (Number(_this.userInfo.as_diquId) === 39) {
                ...
                if (_this.userInfo.as_areaId === null || _this.userInfo.as_areaId === undefined) {
                    params = {
                        topTab: 0,
                        logType: 1,
                        userNo: Number(this.userInfo.as_cardNo)
                    }
                } else {
                    params = {
                        topTab: 0,
                        logType: 1,
                        userNo: '',
                        areaID: _this.userInfo.as_areaId === '' ? _this.userInfo.as_diquId : _this.userInfo.as_areaId
                    }
                }
            }

            // 接口获取数据，对项目阶段进行转换
            getListSupervisorOrder(params).then(results => {
