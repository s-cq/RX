...
<div class="layerRtb layerRtb-threecolumn layerRtb-right">
    <three-title :title="{name:'归还确认'}" :close="false">
        ...
        <div class="analyItem"  v-for="(item, index) of protectList" :key="index" >
            ...
            <div class="analyItemCon">
                <div class="fixedBox  flex">
                    ...
                    <el-checkbox  v-if=" item.isSendBack === 1 " checked="" v-model="checkID[item.id]" @change="selctChange(item.id,index)"></el-checkbox>
                    <el-checkbox  v-else v-model="checkID[item.id]" @change="selctChange(item.id)"></el-checkbox>
                </div>
            </div>
        </div>
    </div>
...
import { getStartingItem, updateGroundbreakingCeremony } from '../../../../Resources/Api/index'
export default {
    props: ['type', 'role'],
    data () {
        return {
        ...
        load () {
            ...
            getStartingItem(parm).then((result) => {
                if (result.data.StatusCode === 1) {
                    ...
                    _this.protectList = cereData.filter((item) => { return item.isSeceive === 1 })
                }
            }).catch((error) => {
                console.log(error)
            })
