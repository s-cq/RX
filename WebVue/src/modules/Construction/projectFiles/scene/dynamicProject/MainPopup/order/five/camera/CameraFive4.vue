...
<div class="layerRtb layerRtb-threecolumn layerRtb-right">
    <three-title :title="{name:'拍照'}" :close="false">
        <span tag="i" class="rig_close fr" @click="$parent.$parent.clickFiveShow()"></span>
    </three-title>
    <ul class="clearfix uiTab8 mb20 ml10" >
        <li :class="['fl', currentIndex === index ? 'uiTab8-active': '']" @click="clickTab(index,item)"  v-for="(item, index) of pointsList" :key="index">{{item.name === null ? `点位 ${index + 1}` : item.name}}</li>
    </ul>
...
import { getPointByPro, getImgStatisticsByPoi, getImgListByDate, addManualCapture } from '../../../../Resources/Api/index'
export default {
    props: ['type', 'role'],
    data () {
        return {
            pointsList: [], // 点位信息
            currentIndex: 0, // 当前index 值
            photoList: [], // 拍照信息列表
            photoTime: '',
            currentCamera: '' // 当前的监控
    ...
    methods: {
        ...
        getPointByPro () {
            ...
            getPointByPro(params).then((result) => {
                if (result.data.StatusCode === 0) {
                    ...
                    this.currentCamera = this.pointsList[0]
                    this._getImgStatisticsByPoiFn()
                }
            }).catch((err) => {
        ...
        _getImgStatisticsByPoiFn () {
            let params = {
                camera: this.currentCamera.camera
            }
        ...
        _getImgListByDateFn (createDate, index) {
            let params = {
                camera: this.currentCamera.camera, // 监控编号
        ...
        _addManualCaptureFn () {
            let params = {
                'camera': this.currentCamera.camera, // 监控点编号
                'region': this.leftInfo.region_id, // 地区编号
                'orderNo': this.leftInfo.order_no, // 单号
                'poiId': this.currentCamera.id // 点位
            }
