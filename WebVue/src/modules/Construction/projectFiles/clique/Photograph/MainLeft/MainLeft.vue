    ...
    <div class="contentbox">
        <div class="dailyftSearch clearfix" v-if="souFlag" style="z-index:2">
            <div class="fl uiHas-textIcon">
                <input type="text" placeholder="卡号/地区/状态" class="col-md-12 searchName" v-model="searchText"  @keyup.enter="selectFilter()">
...
import { getCamerasListByRegion } from '../../../branches/Department/Resources/Api/index'
export default {
    data () {
        return {
            loading: false,
    ...
    methods: {
        ...
        selectFilter () {
            ...
            if (this.searchText === '在线') {
                this.searchText = 0
            } else if (this.searchText === '不在线') {
                this.searchText = 1
            }
            ...
            this.leftListData = this.leftListDataClone.filter(item => (
                searchReg.test(item.card) || searchReg.test(item.region_name) || searchReg.test(item.state)
            ))
