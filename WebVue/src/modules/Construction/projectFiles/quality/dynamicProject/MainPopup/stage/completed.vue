<template>
<div class="layerRtb layerRtb-threecolumn">
    <three-title :title="{name:'竣工'}"></three-title>
    <div class="layerRtb-scroll thinScroll" v-scrollHeight = "137">
        <div class="analyItem">
            <p class="analyItemTit tx-center">竣工</p>
            <div class="analyItemCon">
                <p class="col-md-3">
                    <span class="cLightGray pr8">状态</span>
                    <span class="cRed">未申请</span>
                </p>
            </div>
        </div>
        <div class="analyItem">
            <p class="analyItemTit tx-center">总检</p>
            <div class="analyItemCon">
                <p class="col-md-3">
                    <span class="cLightGray pr8">监理</span>
                    <span class="cRed">未完</span>
                </p>
                <p class="col-md-3">
                    <span class="cLightGray pr8">整改</span>
                    <span class="cRed">未启动</span>
                </p>
            </div>
        </div>
        <div class="analyItem">
            <p class="analyItemTit tx-center">验收</p>
            <div class="analyItemCon">
                <p class="col-md-3">
                    <span class="cLightGray pr8">客户</span>
                    <span class="cRed">未验收</span>
                </p>
            </div>
        </div>
        <div class="analyItem">
            <p class="analyItemTit tx-center">结算</p>
            <div class="analyItemCon">
                <p class="col-md-3">
                    <span class="cLightGray pr8">增减项</span>
                    <span class="cGreen">已审批</span>
                </p>
                <p class="col-md-3">
                    <span class="cLightGray pr8">材料</span>
                    <span class="cGreen">已完成</span>
                </p>
            </div>
        </div>
    </div>
    <div class="layerRtb-footer">
        <div class="analyItem">
            <p class="analyItemTit tx-center">综合</p>
            <div class="analyItemCon">
                <p class="fl col-md-9"></p>
                <p class="fr col-md-3">
                    <span class="cLightGray">统计</span>
                    <span class="cGreen fz14 bold">0分</span>
                </p>
                <p class="fl"><span class="circlemark circlemark-green">优</span></p>
            </div>
        </div>
        <div class="tx-center">
            <input type="button" class="uiBtn-normal uiBtn-blue" value="确定">
        </div>
    </div>
</div>
</template>
<script>
export default {
    data () {
        return {

        }
    },
    created () {

    },
    methods: {

    }
}
</script>
